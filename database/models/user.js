...
class User {
  ...
  static async findOne({ discordId }) {
    try {
      ...
      // Parse guild data - handle errors gracefully
      const guilds = [];
      for (const row of guildRows) {
        try {
          if (typeof row.guildData === 'string') {
            guilds.push(JSON.parse(row.guildData));
          } else {
            guilds.push(row.guildData);
          }
        } catch (err) {
    ...
    try {
      ...
      if (data.guilds && data.guilds.length > 0) {
        for (const guild of data.guilds) {
          const guildDataString = JSON.stringify(guild);
          await connection.query(
            'INSERT INTO user_guilds (userId, guildId, guildData) VALUES (?, ?, ?)',
            [userId, guild.id, guildDataString]
          );
        }
      }
    ...
    try {
      ...
      if (this.guilds && this.guilds.length > 0) {
        for (const guild of this.guilds) {
          ...
          await connection.query(
            'INSERT INTO user_guilds (userId, guildId, guildData) VALUES (?, ?, ?)',
            [this.id, guild.id, guildDataString]
          );
        }
      }
