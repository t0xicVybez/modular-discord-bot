...
class Guild {
  ...
  async save() {
    ...
    try {
      ...
      // For debugging
      console.log('Guild save method called with settings:', JSON.stringify(this.settings));
  
      // Update basic guild info
      await connection.query(
        `UPDATE guilds SET
         name = ?,
         icon = ?,
        ...
        [
          ...
          this.icon,
          this.prefix,
          this.settings.welcomeEnabled ? 1 : 0, // Convert boolean to 0/1 for MySQL
          this.settings.welcomeChannel,
          this.settings.welcomeMessage,
          this.settings.autoRoleEnabled ? 1 : 0, // Convert boolean to 0/1 for MySQL
      ...
      console.log('SQL update completed for guild settings');
