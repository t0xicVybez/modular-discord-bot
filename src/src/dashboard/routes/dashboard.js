...
router.post('/server/:id/settings', isLoggedIn, async (req, res) => {
  try {
    ...
    // Update settings based on form data with proper type conversion
    guildSettings.prefix = prefix || '!';
    
    // For nested objects in Mongoose, direct assignment is better
    guildSettings.settings = {
      ...guildSettings.settings, // Keep existing settings not being updated
      welcomeEnabled: welcomeEnabled,
      welcomeChannel: welcomeChannel || '',
      welcomeMessage: welcomeMessage || 'Welcome {user} to {server}!',
      autoRoleEnabled: autoRoleEnabled,
      autoRoleId: autoRoleId || '',
      // Preserve existing arrays and maps
      disabledCommands: guildSettings.settings.disabledCommands || [],
      permissions: guildSettings.settings.permissions || new Map()
    };
    
    // Log the settings that will be saved
    ...
    // Make sure settings is properly marked as modified
    guildSettings.markModified('settings');

    // Save settings
    await guildSettings.save();
    
    // Verify settings were saved by fetching from DB again
